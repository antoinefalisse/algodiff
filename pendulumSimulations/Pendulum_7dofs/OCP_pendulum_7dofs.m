%%  Simulation of an inverted 7-dof pendulum subject to a perturbation.
    ...
    case 'AD'
        if strcmp(Options.type,'Recorder')
            if strcmp(Options.Hessian,'exact')
                F = external('F','Pendulum_7dofs_exactHessian.dll'); 
            elseif strcmp(Options.Hessian,'approx')
                F = external('F','Pendulum_7dofs.dll'); 
            end
        else
            % ADOL-C cases not available
        end
    case 'FD_F'
        F = external('F','Pendulum_7dofs.dll',struct('enable_fd',true,...
...
pathResults = [pathRepo,'/Results/Pendulum_7dofs/'];
