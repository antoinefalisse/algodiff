%%  Simulation of an inverted 10-dof pendulum subject to a perturbation.
...
Options.N=100; % Number of mesh interval.
    ...
    case 'AD'
        if strcmp(Options.type,'Recorder')
            if strcmp(Options.Hessian,'exact')
                F = external('F','Pendulum_10dofs_exactHessian.dll'); 
            elseif strcmp(Options.Hessian,'approx')
                F = external('F','Pendulum_10dofs.dll'); 
            end
        else
            % ADOL-C cases not available
        end
    case 'FD_F'
        F = external('F','Pendulum_10dofs.dll',struct('enable_fd',true,...
...
pathResults = [pathRepo,'/Results/Pendulum_10dofs/'];
