%%  Simulation of an inverted 8-dof pendulum subject to a perturbation.
    ...
    case 'AD'
        if strcmp(Options.type,'Recorder')
            if strcmp(Options.Hessian,'exact')
                F = external('F','Pendulum_8dofs_exactHessian.dll'); 
            elseif strcmp(Options.Hessian,'approx')
                F = external('F','Pendulum_8dofs.dll'); 
            end
        else
            % ADOL-C cases not available
        end
    case 'FD_F'
        F = external('F','Pendulum_8dofs.dll',struct('enable_fd',true,...
...
pathResults = [pathRepo,'/Results/Pendulum_8dofs/'];
