%%  Simulation of an inverted 3-dof pendulum subject to a perturbation.
    ...
    case 'AD'
        if strcmp(Options.type,'Recorder')
            if strcmp(Options.Hessian,'exact')
                F = external('F','Pendulum_3dofs_exactHessian.dll'); 
            elseif strcmp(Options.Hessian,'approx')
                F = external('F','Pendulum_3dofs.dll'); 
            end
        else
            % ADOL-C cases not available
        end
    case 'FD_F'
        F = external('F','Pendulum_3dofs.dll',struct('enable_fd',true,...
...
pathResults = [pathRepo,'/Results/Pendulum_3dofs/'];
...
% plot(tgrid, x_opt(:,2), '-')
% plot(tgrid, x_opt(:,3), '-')
% stairs(tgrid, [uT_opt; nan nan nan], '-.')
% stairs(tgrid, [ua_opt; nan nan nan], '-.')
% xlabel('t')
% legend('x1','x2','3','uT1','uT2','uT3','ua1','ua2','ua3')
