%%  Simulation of an inverted 2-dof pendulum subject to a perturbation.
    ...
    case 'AD'
        if strcmp(Options.type,'Recorder')
            if strcmp(Options.Hessian,'exact')
                F = external('F','Pendulum2dof_osim_exactHessian.dll'); 
            elseif strcmp(Options.Hessian,'approx')
                F = external('F','Pendulum2dof_osim.dll'); 
            end
        else
            % ADOL-C cases not available
        end
    case 'FD_F'
        F = external('F','Pendulum_2dofs.dll',struct('enable_fd',true,...
...
% States [q pendulum1, qdot pendulum1, q pendulum2, qdot pendulum2]
X0  = MX.sym('X0', 2*nq);
w   = {w{:}, X0};
...
for k=0:N-1
    ...
    % Add path constraint (implicit formulation)
    ...
    w0  = [w0; [IG(i_IG).q((k+1)*(d+1)+1,:)';...
        IG(i_IG).qdot((k+1)*(d+1)+1,:)']./scaling.states];
    Xk_nsc=Xk.*scaling.states;
...
pathResults = [pathRepo,'/Results/Pendulum_2dofs_osim/'];
...
% plot(tgrid, x_opt(:,1), '--')
% plot(tgrid, x_opt(:,2), '-')
% stairs(tgrid, [uT_opt; nan nan], '-.')
% stairs(tgrid, [ua_opt; nan nan], '-.')
% xlabel('t')
% legend('x1','x2','uT1','uT2','ua1','ua2')
...
%    q1=x_opt(i,1);
%    q2=x_opt(i,3);
%    plot([0  -l1*sin(q1) -l1*sin(q1)-l2*sin(q1+q2)],...
%        [0 cos(q1)*l1 l1*cos(q1)+l2*cos(q1+q2)]);
%    xlim([-1 1]);
%    ylim([0 2]);
%    pause(0.5);
%    clf;
...
% xlabel('Design variables','Fontsize',28)
% ylabel('Constraints','Fontsize',28)
% title('Jacobian','Fontsize',30)
% set(gca,'Fontsize',16)

%% Plot Hessian
% figure()
% temp = vertcat(g{:});
% lam = MX.sym('lam', temp.sparsity());
% L = J + dot(lam, temp);
