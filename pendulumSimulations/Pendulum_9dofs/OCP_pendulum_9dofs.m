%%  Simulation of an inverted 9-dof pendulum subject to a perturbation.
...
Options.N=50; % Number of mesh interval.
    ...
    case 'AD'
        if strcmp(Options.type,'Recorder')
            if strcmp(Options.Hessian,'exact')
                F = external('F','Pendulum_9dofs_exactHessian.dll'); 
            elseif strcmp(Options.Hessian,'approx')
                F = external('F','Pendulum_9dofs.dll'); 
            end
        else
            % ADOL-C cases not available
        end
    case 'FD_F'
        F = external('F','Pendulum_9dofs.dll',struct('enable_fd',true,...
...
pathResults = [pathRepo,'/Results/Pendulum_9dofs/'];
