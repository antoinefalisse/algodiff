...
Qs_opt_2D(ww_2D(k)).m = ...
    ...
    (['InitialGuess_',num2str(IGi)]).q_opt_GC;
% Joints velocities
Qdots_opt_2D(ww_2D(k)).m = ...
    Results_2D.(['Derivative_',setup.derivatives]). ...
    (['Hessian_',hess_names{hessi}]). ...
    (['LinearSolver_',linear_solvers{linsoli}]). ...    
    (['MetabolicEnergyRate_',num2str(exp_E)]). ...
    (['InitialGuess_',num2str(IGi)]).qdot_opt_GC;
% Muscle activations
Acts_opt_2D(ww_2D(k)).m = ...
    Results_2D.(['Derivative_',setup.derivatives]). ...
    (['Hessian_',hess_names{hessi}]). ...
    (['LinearSolver_',linear_solvers{linsoli}]). ...    
    (['MetabolicEnergyRate_',num2str(exp_E)]). ...
    (['InitialGuess_',num2str(IGi)]).a_opt_GC;
% Joint torques
Ts_opt_2D(ww_2D(k)).m = ...
    Results_2D.(['Derivative_',setup.derivatives]). ...
    (['Hessian_',hess_names{hessi}]). ...
    (['LinearSolver_',linear_solvers{linsoli}]). ...    
    (['MetabolicEnergyRate_',num2str(exp_E)]). ...
    (['InitialGuess_',num2str(IGi)]).tau_opt_GC;
% Ground reaction forces
GRFs_opt_2D(ww_2D(k)).m = ...
    Results_2D.(['Derivative_',setup.derivatives]). ...
    (['Hessian_',hess_names{hessi}]). ...
    (['LinearSolver_',linear_solvers{linsoli}]). ...    
    (['MetabolicEnergyRate_',num2str(exp_E)]). ...
    (['InitialGuess_',num2str(IGi)]).GRF_opt_GC;
% Stats from CasADi
Stats_2D(ww_2D(k)).m = ...
    Results_2D.(['Derivative_',setup.derivatives]). ...
    (['Hessian_',hess_names{hessi}]). ...
