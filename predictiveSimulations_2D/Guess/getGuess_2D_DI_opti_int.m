...
% Date: 9/9/2019
% 
function guess = getGuess_2D_DI_opti_int(Qs,nq,N,time_IC,NMuscle,jointi,scaling,d)
...
dx = guess.QsQdots(end,2*jointi.pelvis.tx-1) - guess.QsQdots(end-1,2*jointi.pelvis.tx-1);
inv_X(2*jointi.pelvis.tx-1) = guess.QsQdots(end,2*jointi.pelvis.tx-1) + dx;            
...
guess.a_b = [guess.a_b; guess.a_b(1,1)];
...
guess.a_b_col = zeros(d*N,nq.trunk);
...
for k=1:N
    ...
    guess.a_b_col((k-1)*d+1:k*d,:) = repmat(guess.a_b(k,:),d,1);
