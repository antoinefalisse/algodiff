...
if solveProblem
    ...
    for k=0:N-1
        ...
        [Hilldiffk,FTk,Fcek,Fpassk,Fisok,vMmaxk,massMk] = ...
            f_forceEquilibrium_FtildeState(ak,FTtildek.*scaling.FTtilde',...
        ...
        if W.mE ~= 0    
            ...
            [~,lMtildek] = f_FiberLength_TendonForce(...
                FTtildek.*scaling.FTtilde',lMTk_lr); 
            % Get muscle fiber velocities
            [vMk,~] = f_FiberVelocity_TendonForce(FTtildek.*scaling.FTtilde',...
        ...
        for j=1:d
            ...
            elseif W.act == 0
                J = J + 1/(dist_trav_tot)*(...
                    W.mE*B(j+1)     *(f_sumsqr_exp(e_tot,exp_E))*h + ...
                    W.back*B(j+1)   *(sumsqr(e_bk))*h +... 
                    W.acc*B(j+1)    *(sumsqr(Ak))*h + ...                          
                    W.u*B(j+1)      *(sumsqr(vAk))*h + ...
                    W.u*B(j+1)      *(sumsqr(dFTtildek))*h);  
        ...
        lbg             = [lbg; zeros(NMuscle,1)];
        ubg             = [ubg; zeros(NMuscle,1)];   
        % We impose periodicity and symmetry and create some helper variables to
        % easily impose these constraints. Here we use these variables when
        % settings initial guesses of states at the end of the current interval.
        % Muscles
...
if analyseResults
    ...
    NControls = NMuscle+NMuscle+nq.all+nq.trunk;
    ...
    for i = 1:nq.all
        qdotdot_opt(:,i) = w_opt(NParameters+NStates+NMuscle+NMuscle+i:Nwl:Nw);
    end
    % Back excitations
    ...
    for nmusi=1:NMuscle
        ...
        a_opt_ext(4:(d+1):end,nmusi) = w_opt(Nwm+NMuscle+NMuscle+nmusi:Nwl:Nw);
    end  
    % Muscle-tendon forces
    FTtilde_opt_ext=zeros(N*(d+1)+1,NMuscle);
    FTtilde_opt_ext(1:(d+1):end,:)= FTtilde_opt;
    for nmusi=1:NMuscle
        FTtilde_opt_ext(2:(d+1):end,nmusi) = w_opt(Nwm+d*NMuscle+nmusi:Nwl:Nw);
    ...
    qdotdot_opt_unsc.deg(:,dof_roti) = qdotdot_opt_unsc.deg(:,dof_roti).*180/pi;
    % Time derivative of muscle activations (states)
    ...
    assertBackTmax = ...
        max(max(abs(out_res_opt(:,jointi.trunk.ext)/scaling.BackTau - ...
            (a_b_opt_unsc))));
    if assertBackTmax > 1*10^(-tol_ipopt)
    ...
    %% Compute metabolic cost of transport (COT) over gait cycle
    q_opt_GC_rad = q_opt_GC;
    q_opt_GC_rad(:,dof_roti) = q_opt_GC_rad(:,dof_roti).*pi/180;
    qdot_opt_GC_rad = qdot_opt_GC;
    qdot_opt_GC_rad(:,dof_roti)= qdot_opt_GC_rad(:,dof_roti).*pi/180; 
    % Pre-allocations
    e_mo_opt = zeros(2*N,1);     
    for nn = 1:2*N
        % Get muscle-tendon lengths and velocities
        % Left leg
        qin_l_opt = [q_opt_GC_rad(nn,jointi.hip.l),...
            q_opt_GC_rad(nn,jointi.knee.l),q_opt_GC_rad(nn,jointi.ankle.l)];  
        qdotin_l_opt = [qdot_opt_GC_rad(nn,jointi.hip.l),...
        ...
        % Right leg
        qin_r_opt = [q_opt_GC_rad(nn,jointi.hip.r),...
            q_opt_GC_rad(nn,jointi.knee.r),q_opt_GC_rad(nn,jointi.ankle.r)];  
        qdotin_r_opt = [qdot_opt_GC_rad(nn,jointi.hip.r),...
        ...
        % Both legs
        lMTk_lr_opt = [lMTk_l_opt;lMTk_r_opt];
        vMTk_lr_opt = [vMTk_l_opt;vMTk_r_opt];   
        % Get muscle-tendon forces and derive Hill-equilibrium
        [~,~,Fce_opt,Fpass_opt,Fiso_opt,vMmax_opt,massM_opt] = ...
            f_forceEquilibrium_FtildeState(a_opt_GC(nn,:),...
            FTtilde_opt_GC(nn,:),dFTtilde_opt_GC(nn,:),lMTk_lr_opt,...
            vMTk_lr_opt,tensions); 
        [~,lMtilde_opt] = f_FiberLength_TendonForce(FTtilde_opt_GC(nn,:)',...
            full(lMTk_lr_opt));                  
        [vM_opt,~] = ...
            f_FiberVelocity_TendonForce(FTtilde_opt_GC(nn,:)',...
            dFTtilde_opt_GC(nn,:)',full(lMTk_lr_opt),full(vMTk_lr_opt));
        % Bhargava et al. (2004)
        [~,~,~,~,~,e_mot] = ...
            fgetMetabolicEnergySmooth2004all(e_opt_GC(nn,:)',...
            a_opt_GC(nn,:)',full(lMtilde_opt),full(vM_opt),...
            full(Fce_opt),full(Fpass_opt),full(massM_opt),pctsts,...
            full(Fiso_opt),MTparameters_m(1,:)',body_mass,10);        
        e_mo_opt(nn,:) = full(e_mot)'; 
    end
    % Get COT
    dist_trav_opt_GC = q_opt_GC_rad(end,jointi.pelvis.tx) - ...
        q_opt_GC_rad(1,jointi.pelvis.tx); % distance traveled    
    time_GC = q_opt_GUI_GC(:,1);
    e_mo_opt_tr_GC = trapz(time_GC,e_mo_opt);
    % Cost of transport: J/kg/m
    % Energy model from Bhargava et al. (2004)
    COT_opt_GC = e_mo_opt_tr_GC/body_mass/dist_trav_opt_GC;     
    ...
    if saveResults
        ...
        if (exist([pathresults,'/',namescript,'/Results_2D.mat'],'file')==2) 
            ...
            Results_2D.(['Derivative_',setup.derivatives]). ...
                ...
                (['InitialGuess_',num2str(IGi)]) = struct('q_opt',[]);
        end            
        ...
        Results_2D.(['Derivative_',setup.derivatives]). ...
            ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).q_opt_GC = q_opt_GC;
        Results_2D.(['Derivative_',setup.derivatives]). ...
            (['Hessian_',hess_names{hessi}]). ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).qdot_opt_GC = qdot_opt_GC;
        Results_2D.(['Derivative_',setup.derivatives]). ...
            (['Hessian_',hess_names{hessi}]). ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).qdotdot_opt_GC = qdotdot_opt_GC;
        Results_2D.(['Derivative_',setup.derivatives]). ...
            (['Hessian_',hess_names{hessi}]). ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).GRF_opt_GC = GRF_opt_GC;
        Results_2D.(['Derivative_',setup.derivatives]). ...
            (['Hessian_',hess_names{hessi}]). ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).tau_opt_GC = tau_opt_GC;
        Results_2D.(['Derivative_',setup.derivatives]). ...
            (['Hessian_',hess_names{hessi}]). ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).a_opt_GC = a_opt_GC;
        Results_2D.(['Derivative_',setup.derivatives]). ...
            (['Hessian_',hess_names{hessi}]). ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).FTtilde_opt_GC = FTtilde_opt_GC;
        Results_2D.(['Derivative_',setup.derivatives]). ...
            (['Hessian_',hess_names{hessi}]). ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).e_opt_GC = e_opt_GC;
        Results_2D.(['Derivative_',setup.derivatives]). ...
            (['Hessian_',hess_names{hessi}]). ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).dFTtilde_opt_GC = dFTtilde_opt_GC;
        Results_2D.(['Derivative_',setup.derivatives]). ...
            (['Hessian_',hess_names{hessi}]). ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).a_b_opt_GC = a_b_opt_GC;
        Results_2D.(['Derivative_',setup.derivatives]). ...
            (['Hessian_',hess_names{hessi}]). ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).stats = stats;
        Results_2D.(['Derivative_',setup.derivatives]). ...
            (['Hessian_',hess_names{hessi}]). ...
            (['LinearSolver_',linear_solvers{linsoli}]). ...
            (['MetabolicEnergyRate_',num2str(exp_E)]). ...
            (['InitialGuess_',num2str(IGi)]).COT_opt = COT_opt_GC;
        % Save data
